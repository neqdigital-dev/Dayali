...
export default function PrivateCard({ dragHandleProps }: { dragHandleProps?: any }) {
  const { t, i18n } = useTranslation(['dashboard', 'common']);
  const lang = i18n.language as 'pt' | 'en';
  
  const { agendaEvents, addAgendaEvent, addSubtopic, toggleSubtopic, deleteSubtopic, deleteAgendaEvent, updateAgendaEvent } = useDataStore();
  
  const privateEvents = agendaEvents.filter(e => e.category === 'private' && !e.completed);
  
  // Sorting and filtering
  const sortedEvents = [...privateEvents].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
  ...
  const completedCount = privateEvents.reduce((acc, ev) => acc + (ev.subtopics?.filter(s => s.completed).length || 0), 0);
  const totalCount = privateEvents.reduce((acc, ev) => acc + (ev.subtopics?.length || 0), 0);
  const progress = totalCount > 0 ? (completedCount / totalCount) * 100 : 0;

  return (
    <div className="card card-category private">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', width: '100%', gap: 'var(--space-2)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span className="badge badge-private" style={{ gap: '4px' }}>
                <User size={14} /> {t('category.private', { ns: 'common' })}
              </span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
              <span className="task-column-count text-sm text-tertiary">{completedCount}/{totalCount}</span>
              <button className="btn-icon" title={t('actions.add', { ns: 'common' })} onClick={() => setEditingEvent({ category: 'private' })}>
                <Plus size={16} />
              </button>
            </div>
          </div>
          <div style={{ height: 4, background: 'var(--color-bg-subtle)', borderRadius: 2, width: '100%', overflow: 'hidden' }}>
            <div style={{ height: '100%', background: 'var(--color-private)', width: `${progress}%`, transition: 'width 0.3s ease' }} />
          </div>
        </div>
      </div>
